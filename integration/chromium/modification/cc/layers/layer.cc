...
void Layer::SetHitTestOpaqueness(HitTestOpaqueness opaqueness) {
  // See SetHideLayerAndSubtree: same re-entrant paint guard.
  if (!IsPropertyChangeAllowed()) return;
  auto& inputs = inputs_.Write(*this);
...
void Layer::SetHideLayerAndSubtree(bool hide) {
  // During in_paint_layer_contents(), Win32 WM_ACTIVATE can arrive
  // re-entrantly (e.g. from ShowWindow()), triggering focus-ring visibility
  // changes. Silently skip: the focus ring re-evaluates on the next frame.
  if (!IsPropertyChangeAllowed()) return;
  auto& inputs = EnsureLayerTreeInputs();
