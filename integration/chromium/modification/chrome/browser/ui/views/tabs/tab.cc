...
void Tab::RspProxyButtonPressed(const ui::Event& event) {
  Profile* profile = GetRspProfileForTab(tab_handle_);
  if (!profile) {
    return;
  }
  // PostTask so the button-press event fully unwinds before the bubble
  // widget is created and steals focus.  Showing the bubble synchronously
  // from inside an event handler causes a mid-paint compositor activation
  // change that triggers a DCHECK(IsPropertyChangeAllowed()) in
  // cc::Layer::SetHideLayerAndSubtree via the focus-ring visibility update.
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
      FROM_HERE,
      base::BindOnce(&RspToolbarButton::ShowBubbleForProfile,
                     base::Unretained(rsp_proxy_button_),
