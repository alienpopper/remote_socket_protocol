...
void NewRspTab(BrowserWindowInterface* browser, const GURL& url) {
  ...
  // Use a shared RSP OTR profile so Navigate can find/reuse the same RSP
  // window across multiple NewRspTab calls (analogous to how Chrome reuses
  // the incognito window). OTRProfileID caches by ID, so the same profile
  // instance is returned on subsequent calls.
  Profile* rsp_profile = browser->GetProfile()->GetOriginalProfile()
      ->GetOffTheRecordProfile(
          Profile::OTRProfileID::CreateForSharedRspWindow(),
...
content::WebContents& NewTab(BrowserWindowInterface* browser,
  ...
  // The call to AddAndReturnTabAt above did not set the focus to the tab as its
