 ...
 public:
  ...
  class OTRProfileID {
   public:
    ...
    // Creates an OTR profile ID for an RSP tab.
    static OTRProfileID CreateUniqueForRspTab();

    // Returns a fixed (non-unique) OTR profile ID for the shared RSP window.
    // NewRspTab reuses the same window; use CreateUniqueForRspTab for
    // independent RSP windows via NewRspWindow.
    static OTRProfileID CreateForSharedRspWindow();

    // Returns true if this is an RSP tab OTR profile.
    bool IsRspTab() const;
