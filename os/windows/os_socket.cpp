...
int sendSocketTo(SocketHandle socketHandle, const uint8_t* data, uint32_t length,
    ...
    return sendto(toNative(socketHandle),
                  reinterpret_cast<const char*>(data), static_cast<int>(length), 0,
                  reinterpret_cast<const sockaddr*>(destination.bytes.data()),
                  static_cast<int>(destination.length));
}

int recvSocketFrom(SocketHandle socketHandle, uint8_t* buffer, uint32_t length,
                   PeerAddress& source) {
    source = {};
    int srcLen = static_cast<int>(source.bytes.size());
    const int result = recvfrom(toNative(socketHandle),
                                reinterpret_cast<char*>(buffer), static_cast<int>(length), 0,
...
std::string peerAddressKey(const PeerAddress& address) {
    ...
    if (sa->sa_family == AF_INET6) {
        ...
        std::string key(18, '\0');
        std::memcpy(key.data(), &sin6->sin6_addr, 16);
...
std::string peerAddressString(const PeerAddress& address) {
    ...
    if (sa->sa_family == AF_INET) {
        const auto* sin = reinterpret_cast<const sockaddr_in*>(sa);
        InetNtopA(AF_INET, &sin->sin_addr, hostBuf, sizeof(hostBuf));
        port = ntohs(sin->sin_port);
    } else if (sa->sa_family == AF_INET6) {
        const auto* sin6 = reinterpret_cast<const sockaddr_in6*>(sa);
        InetNtopA(AF_INET6, &sin6->sin6_addr, hostBuf, sizeof(hostBuf));
