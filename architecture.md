...
------------------------
The endorsement services are used to assign properties and authentications to nodes.  For example, to access the ssh servers in lab12, you may need an endorsement from the lab12 ES. Any type of information can be bound via an ES.
...
The Security Model
------------------------
The RM only provides authN/authZ for if a CS/RM/RS is allowed to attach to the services.  This is to stop random outsiders from probing the services and to stop denial-of-service type situations.  The RM does not provice encryption or integrity protections.  If the protocol needs encryption and integrity, it needs to be supplied by that protocol.  The RM is only an insecure transport mechanism.

Whether a resource can be used by a particular client is negotiated between the resource and the client.  All messages generated by CS and RS nodes are authenticated.  All RS should always know who they are doing actions on behalf of.  All CS nodes should always know who is providing access.

The RM transport layers are allowed to reboot/reconnect/reconfigure in real time without interrupting established sessions between a CS and a RS.  A client is allowed to connected to one RM, start a session, and then switch that session to another RM or a different transport protocol.  The per-message protections need to be strong enough to make this OK.
