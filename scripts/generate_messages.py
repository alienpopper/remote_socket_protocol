#!/usr/bin/env python3
"""Generate language-specific message schema modules from .proto files.
...
def generate_rust(enums, messages, output_path):
    schema = {"enums": enums, "messages": messages}
    schema_json = json.dumps(schema, indent=4)

    lines = [
        "// Generated from proto files by scripts/generate_messages.py.",
        "// Do not edit this file directly.",
        "",
        f'pub const SCHEMA_JSON: &str = r#"{schema_json}"#;',
        "",
        "pub mod enum_values {",
    ]

    for enum_name, values in enums.items():
        mod_name = _to_snake_case(enum_name)
        lines.append(f"    pub mod {mod_name} {{")
        for value_name, value_num in values.items():
            lines.append(f"        pub const {value_name}: i32 = {value_num};")
        lines.append("    }")
        lines.append("")
    ...
    for enum_name, values in enums.items():
        rust_enum_name = _to_pascal_case(enum_name)
        lines.extend([
            "#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]",
            "#[repr(i32)]",
            f"pub enum {rust_enum_name} {{",
        ])
        for value_name, value_num in values.items():
            lines.append(f"    {_to_pascal_case(value_name)} = {value_num},")
        lines.append("}")
        lines.append("")
        lines.extend([
            f"impl {rust_enum_name} {{",
            "    pub fn from_i32(value: i32) -> Option<Self> {",
            "        match value {",
        ])
        for value_name, value_num in values.items():
            lines.append(f"            {value_num} => Some(Self::{_to_pascal_case(value_name)}),")
        lines.extend([
            "            _ => None,",
            "        }",
            "    }",
            "}",
            "",
            f"impl From<{rust_enum_name}> for i32 {{",
            f"    fn from(value: {rust_enum_name}) -> Self {{",
            "        value as i32",
            "    }",
            "}",
            "",
...
def main():
    ...
    parser.add_argument("--nodejs", help="Output path for messages.js")
    parser.add_argument("--python", help="Output path for messages.py")
    parser.add_argument("--rust", help="Output path for generated Rust schema source")
    args = parser.parse_args()

    if not (args.nodejs or args.python or args.rust):
        parser.error("At least one output must be specified (--nodejs, --python, --rust).")
    ...
    if args.rust:
        generate_rust(enums, messages, args.rust)
        print(f"Generated {args.rust}", file=sys.stderr)
